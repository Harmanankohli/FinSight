...
def _make_query_context(**kw):
    return QueryContext(**{
        "ticker": "AAPL", "user_query": "Analyze Apple",
        "user_risk_profile": "medium", "portfolio_holdings": ["MSFT"],
        "investment_horizon": "medium_term", "session_id": "s1",
        "timestamp": _NOW, **kw,
    })
...
def _make_rag(**kw):
    return RAGInsights(**{
        "ticker": "AAPL", "revenue_growth_yoy": 0.12, "rd_spend_billions": 3.5,
        "forward_guidance": "positive", "key_risks": ["competition"],
        "cited_documents": ["10-K 2024"], "confidence_score": 0.85, **kw,
    })


def _make_quant(**kw):
    return QuantMetrics(**{
        "ticker": "AAPL", "sharpe_ratio": 1.5, "annual_volatility": 0.25,
        "beta": 1.1, "var_95_daily": -0.022,
        "portfolio_correlation": {"MSFT": 0.7},
        "quant_signal": "BUY", "quant_confidence": 0.78, **kw,
    })


def _make_sentiment(**kw):
    return SentimentIntelligence(**{
        "ticker": "AAPL", "social_sentiment_score": 0.6,
        "analyst_consensus": "bullish", "avg_price_target": 200.0,
        "insider_signal": "neutral", "narrative": "Strong product cycle",
        "overall_signal": "BUY", "confidence_score": 0.75,
        "key_risks": ["macro"], "key_catalysts": ["iPhone 17"], **kw,
    })


def _make_brief(**kw):
    return InvestmentBrief(**{
        "ticker": "AAPL", "generated_at": _NOW,
        ...
        "disclaimer": "Not financial advice.", **kw,
    })
