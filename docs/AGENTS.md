...
7. **Phase 2 — Review** — After all Phase 1 responses, call `send_message` for the Reviewer Agent with JSON payload `{"ticker": "...", "session_id": "..."}`. Reviewer fetches full agent outputs from shared SQLite store via `session_id`, runs 4 deterministic tools directly in Python, and produces cross-validation verdict.
...
| **Ticker validation** | `validate_ticker()` and `resolve_ticker()` in `src/shared/ticker_utils.py` use `get_shared_mcp()` internally. Shared across all executors — the private `_validate_ticker` methods were removed. Blocks financial acronyms (SEC, EPS, CEO, etc.) via hardcoded `_COMMON_ACRONYMS` set. |
