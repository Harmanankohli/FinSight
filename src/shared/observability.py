...
def _should_export_span(span: ReadableSpan) -> bool:
    """Drop parentless OpenAI-generation spans (orphan RAGAS eval calls).

    reviewer/agent.py imports ``from langfuse.openai import AsyncOpenAI``
    which globally monkey-patches the openai module.  RAGAS eval LLM calls
    go through this patch and get langfuse-sdk spans.  The eval scoring
    functions attach ROOT_CONTEXT so those spans have no parent — this
    filter drops them while keeping all legitimately-parented spans.
    """
    if not is_default_export_span(span):
        return False
    if span.parent is None and span.name.startswith("OpenAI"):
        return False
    return True
