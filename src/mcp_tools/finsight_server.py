...
Architecture
------------
- ``mcp_tools._app``         : FastMCP singleton (``app``)
- ``mcp_tools.infra``        : rate limiters, caches, news helpers, embed loader
- ``mcp_tools.tools``        : all @app.tool() registrations (imported for side-effects)
"""
...
import mcp_tools.tools  # noqa: F401  (side-effect: registers all tools)
import mcp_tools.tools.agent_registry as _ar_mod
from mcp_tools._app import app  # noqa: E402  (must follow bootstrap)
