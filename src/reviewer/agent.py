...
reviewer_agent = Agent(
    ...
    instructions=(
        ...
        "2. Populate contradictions ONLY from the contradictions input. Each entry must "
        "describe a genuine conflict between two data points. Do NOT create entries for "
        "other inputs (source_verifications, confidence_breakdown, recommendation_validation) "
        "or entries that report agreement — a short list, or an empty one, is correct when "
        "few real conflicts exist.\n"
        ...
        "CRITICAL: Every claim must be directionally consistent with the number it cites. "
        "A DCF intrinsic value BELOW the current price is a bearish/overvaluation signal — "
        "never describe it as 'favorable valuation' or upside. Before finalizing, re-check "
        "each sentence: does the stated interpretation (favorable/unfavorable, bullish/bearish) "
        "match the sign and direction of the cited figure?\n\n"
        "CRITICAL: Cite numbers EXACTLY as they appear in the input, attributed to the agent "
        "they belong to. Never attach one agent's confidence score to a different agent, never "
        "call a score below 50% 'high', and never state a recommendation (buy/hold/sell) that "
        "differs from the verdict you set. If you are not certain which agent a number came "
        "from, omit the number rather than guess."
    ),
